...
must "$SKM" 'private fun rewriteSkillName(file: File, canonicalName: String)'
...
must "$MA" 'width: min(980px, calc(100vw - 20px)) !important'
...
must "$EM" 'preservedRoots = if (preserveHome) setOf(homeDir) else emptySet()'
