...
must "$MA" 'text = "从手机导入到当前工作区"'
must "$MA" 'text = "从手机导入文件（可多选）"'
must "$MA" 'text = "复制手机文件 / 文件夹"'
must "$MA" '手机文件只作为来源；目标始终是当前 DSH 工作目录。'
must "$MA" 'private fun showWorkspaceBulkImportPicker()'
must "$MA" 'private fun showWorkspaceBulkImportPicker(workspace: String)'
must "$MA" 'private fun showWorkspaceImportDialog()'
must "$MA" 'private fun currentWritableWorkspacePath(): String?'
must "$MA" '请先在 DSH 中打开一个工作区。手机文件只会复制到当前工作目录'
forbid "$MA" 'private enum class WorkspaceImportAction { FILES, BULK }'
forbid "$MA" 'pendingWorkspaceImportAction = WorkspaceImportAction.FILES'
forbid "$MA" 'pendingWorkspaceImportAction = WorkspaceImportAction.BULK'
forbid "$MA" '没有当前工作区时会先让你选择导入目标目录，选定后自动继续。'
...
must "$BT" 'DSH_AUTO_INSTALL_HOST_BUILD_DEPS'
must "$BT" 'DSH_PREFLIGHT_ONLY'
must "$BT" ':app:checkDebugAarMetadata'
must "$BT" ':app:compileDebugKotlin'
must "$BT" ':app:processDebugMainManifest'
must "$BT" ':app:mergeDebugResources'
...
must "$BT" 'TERMUX__PREFIX="$stage/usr"'
must "$BT" 'test -x "$TERMUX__PREFIX/libexec/dsh/bash-real"'
must "$ASR" '#!/system/bin/sh'
