- 新增完整 DSH Runtime Graph CI：按正式 Termux 构建相同的 release-family lock 生成 npm lock、安装完整 rc.2 graph、验证已安装 family/Cordis singleton、应用全部 Android runtime patch、对所有 patch 后 JS 执行 `node --check`、验证真实 Agent preset，并实际执行 `dsh web --dump-default-config` + Desktop Web capability contract。该 job 专门用于把完整依赖图组合错误提前到 CI，而不是等手机长构建末尾才发现。
- CI 真实 Agent preset 校验改为 `npm pack @deepseek-ai/dsh-agent-presets@0.1.5-rc.2` 后直接解包发布 tarball，不再让 npm 为独立 preset 包解析开放 peer range。这样避免意外拉到 rc.3/Cordis 4.0.4 造成伪失败，同时仍对官方 rc.2 实际 shipped preset 文件执行完整 parity validator。
- 修复 Agent preset parity validator 的真实 Node API 错误：移除不存在的 `fs.isDirectorySync()/fs.isFileSync()`，统一使用 `fs.statSync(...).isDirectory()/isFile()` 并安全处理 ENOENT。CI 不再只跑 validator 自测，而是实际安装官方 `@deepseek-ai/dsh-agent-presets@0.1.5-rc.2` 并对真实 shipped preset 目录执行完整校验。
- 修复 Android sandbox staged smoke 的 Termux 前缀泄漏：构建校验现在显式使用 staged `TERMUX__PREFIX/PREFIX/HOME/TMPDIR/LD_LIBRARY_PATH/PATH`，不再继承宿主 `/data/data/com.termux/files/usr`，并实际确认 `libexec/dsh/bash-real` 可执行。sandbox runner 改为 Android 原生 `#!/system/bin/sh` shebang，避免 App 运行时依赖不存在的 `/usr/bin/env`。CI 新增 staged Bash wrapper 回归测试，防止该错误再次进入长构建。
- 构建一次性预检增强：`build-termux.sh` 现在会在 Runtime/npm/native 重建前自动补齐缺失的 Termux 宿主构建依赖、自动尝试安装 Android SDK Platform 36，并先执行 Android Gradle/Kotlin/Manifest/resources 快速编译；新增 `DSH_PREFLIGHT_ONLY=1` 快速模式。Gradle Wrapper 网络超时由 10 秒提高到 60 秒，减少移动网络抖动导致的无意义重跑。GitHub Actions 同时执行真实 `:app:assembleDebug`，用占位 Runtime 覆盖依赖解析、Kotlin、Manifest、resources 与 APK 打包阶段。
- Agent Preset Tool Parity：新增 shipped Agent preset 工具契约校验，直接审计 APK Runtime 中 `@deepseek-ai/dsh-agent-presets/presets` 的 `standard / ptc / minimal / cordis` 四套官方 composition。构建现在会确认 Bash、FS/搜索、Jobs、Skills、Goal/Plan、Subagents、Workflow/Ralph、Ask User、Todo、Web、Present，以及 minimal 的 persistent terminal 等关键工具仍存在，并保留 POSIX/Windows gate 与 PTC 的 workflow 禁用语义；防止 Host/Web 本身能启动但模型实际工具集被裁剪或版本漂移。
- Desktop Web Capability Contract：新增构建期桌面能力组成校验。Android Runtime 在加入移动端 MCP overlay 前会对官方 Web `--dump-default-config` 做结构化审计，要求 Workspace/Workspace Files、插件管理、Jobs、Skills、Subagents、Workflow、文件工具、Web 工具、TypeScript Code Runtime、附件与核心 Host/API/UI 服务保持在官方桌面 Web 组成中；同时固定 Web 的 Agent-scoped tool 语义、Windows-only PowerShell gate 与 opt-in Schedule gate，避免后续 DSH 更新或 Android patch 静默“少功能”。该校验只验证官方组成，不强行启用桌面端本来就不默认启用的实验 Python Runtime/Windows PowerShell。
- Android Desktop Backend Parity：修复 Workspace/沙箱/子进程/Terminal 执行链。`dsh-sandbox-local` 在 Android standalone 下使用显式 app-UID partial runner，不再误走桌面 Linux bwrap/Landlock 后 fail-closed；`dsh-subprocess-local` 跳过 systemd/Linux-scope 探测并使用其 fallback containment；`dsh-terminal-bash` 默认解析 APK 内嵌 Bash，不再依赖 `/bin/bash`。`fs-sandbox` 继续作为模型文件写入的 canonical workspace containment。构建会把 runner 作为 Runtime 必需文件，并在 staging、已安装 Runtime health、Web smoke 和 CI 中验证。CI 还会直接安装官方 DSH 0.1.5-rc.2 npm 发布包，确认三个补丁真实命中发布产物。
- 完整审计 Termux 构建预检与 Runtime smoke：新增统一 `dsh_run_tool_smoke()` CLI 契约分派器，修正 `ffmpeg/ffprobe` 使用 `-version`（不再错误使用 `--version`），Host 端在大规模 staging/native 编译前即对全部必需 CLI 做真实启动检查，staged Runtime 复用同一套契约；静态 preflight 现在批量 `bash -n` 所有 shell、`node --check` 所有构建 `.mjs`，并增加已知 CLI 参数回归 guard。新增快速 GitHub Actions `Validate build preflight`，验证 shell/JS 语法、dispatcher 参数映射和历史回归套件；同时清理与当前工作区导入/导出、MCP Web smoke、release-family 实现不一致的陈旧断言。
- 修复 SQLite CLI provider：Termux 的 `libsqlite` 只提供 SQLite 库，`/bin/sqlite3` 由 `sqlite` 子包提供。构建依赖和 staged tool contract 现统一改为 `sqlite3 <- sqlite`，避免 host preflight 在 `/usr/bin/sqlite3` 缺失时失败。
- 修复 Termux 工具依赖闭包在 `set -Eeuo pipefail` 下的误失败：旧 `resolve_termux_package_closure()` 以 `[ installed ] && printf` 作为管道末端，若最后一个候选依赖未安装，`while` 会正常返回 1，却被 ERR trap 误报成 `awk` 构建失败。现在改为显式 `if` 过滤、独立检查 `apt-cache` 查询状态并保证正常过滤路径返回 0；同时扩展 host tool/owner discovery 到新增 `7z/yq/sqlite3/cmake/ninja/ss/dig/magick/ffmpeg/ffprobe/pdfinfo/adb`，避免分包 provider 漏入 staging closure。
- `mobile_tools` 3.2 新增 No-Root Runtime Analysis：默认优先自身/同 UID `/proc`、FD/socket、runtime snapshot/diff 与 child strace；新增内置 Wireless ADB client（`android-tools`），提供 `adb_devices/pair/connect/package_info/process_info/logcat/jdwp_list/pull_apk`。ADB 仅在用户主动开启无线调试并完成配对后工作，不依赖 Root/Shizuku，也不绕过 Android 授权。当前 `mobile_tools` 为 72 个工具且 72 个 handler 一一对应。
...
- 新增独立 `basic_tools` MCP：与现有 `mobile_tools` 隔离并默认同时注册，当前提供 52 个结构化基础工具，覆盖文件/目录、压缩包、JSON/YAML/TOML/XML/CSV、SQLite、文本处理、环境/磁盘、TCP/DNS/TLS、证书/编码、Git、构建检测与 Gradle/npm/CMake、ImageMagick、FFmpeg、PDF、SHA-256 manifest；Runtime 同步加入 `7zip/yq/sqlite/cmake/ninja/iproute2/dnsutils/imagemagick/ffmpeg/poppler`，并增加 staged payload contract、wrapper 与 smoke test。
